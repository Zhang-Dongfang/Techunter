...
The current release requires all migrations in filename order, ending with [`202609120004_task_coordination.sql`](migrations/202609120004_task_coordination.sql). Stop old API task writes, apply the migration, replace API processes, and update Desktop. Submissions now require the UUID `workspaceId` of the current user's running workspace; old clients must be upgraded.
