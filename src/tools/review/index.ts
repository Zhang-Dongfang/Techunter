...
export async function run(_input: Record<string, unknown>, config: TechunterConfig): Promise<string> {
  ...
  try {
    issueNumber = await select({
      ...
      choices: tasks.map((task) => ({
        name: `#${String(task.number).padEnd(4)} @${task.assignee ?? '-'}  ${task.title}`,
        value: task.number,
      })),
    });
  } catch {
  ...
  const selectedTask = tasks.find((task) => task.number === issueNumber)!;
  const expectedHeadBranch = selectedTask.assignee
    ? makeTaskBranchName(selectedTask.number, selectedTask.assignee)
    : undefined;

  const spinner2 = ora(`Loading #${issueNumber}...`).start();
  let pr: Awaited<ReturnType<typeof getTaskPR>>;
  try {
    pr = await getTaskPR(config, issueNumber, expectedHeadBranch);
    spinner2.stop();
  } catch (err) {
    spinner2.stop();
  ...
  if (pr) {
    ...
    console.log(chalk.yellow(`  No open PR found for task #${issueNumber}${expectedHeadBranch ? ` from ${expectedHeadBranch}` : ''}`));
  }
  console.log(`${divider}\n`);
