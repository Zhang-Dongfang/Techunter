...
Techunter is an npm-workspaces monorepo. Desktop calls the central API; neither application imports source code from the other.
...
Cross-application behavior belongs in `@techunter/core`. UI, HTTP handlers, persistence, Electron IPC, and other adapter-specific code stays in its application.
...
```bash
npm install
npm run dev          # local Web + Electron, connected to TECHUNTER_API_URL
npm run dev:api      # central API (separate process)
npm run typecheck    # all workspaces
npm test             # API and local-Agent tests
npm run build        # core, API, then desktop
```
...
- Task metadata and guide formatting come from `@techunter/core` so API and Desktop remain compatible.
- Publishing a desktop task creates a linked GitHub Issue before freezing points.
- Claiming is atomic and recoverable; only one user can win a concurrent claim.
- Claims, releases, reviews, and cancellations share a durable per-task operation. Preserve uncertain merge outcomes; never refund a task while acceptance is pending.
- A task keeps its immutable `workingBranch` across releases and reassignment; accepted child work stays on that branch.
- Submissions identify a running workspace owned by the assignee on the current device. Validate again after model review.
- Project sync and branch switching use conditional writes so a slow response cannot overwrite a newer selection.
