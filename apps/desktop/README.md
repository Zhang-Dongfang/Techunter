...
环境准备、测试和命令台使用统一的命令生命周期管理。Windows 将命令及其后代放进 Job Object，超时、取消及应用退出会清理整个命令进程树；即使原 shell 已退出，后台后代也仍被跟踪。应用退出会等待清理完成。此机制管理进程生命周期，不限制命令的宿主机权限。Windows 回归覆盖真实 PowerShell/Node 的后代退出、延迟写入、无关进程隔离及 UTF-8 输出；其他平台使用进程组终止。

配合迁移 008 与更新后的 API，维护者可在任务详情核对已释放或转交任务的历史交付；恢复先验证原审核快照，只给原提交作者结算，并保留恢复审计。
