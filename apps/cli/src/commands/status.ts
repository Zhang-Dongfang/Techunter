...
import { taskStatusFromLabels } from '@techunter/core';

function getStatus(labels: string[]): { text: string; color: (s: string) => string } {
  const status = taskStatusFromLabels(labels);
  if (status === 'in-review') return { text: status, color: chalk.blue };
  if (status === 'claimed') return { text: status, color: chalk.yellow };
