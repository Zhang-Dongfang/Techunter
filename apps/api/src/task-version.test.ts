...
test('root tasks freeze the latest default branch head', async () => {
  let branchRead = false;
  const version = await resolveTaskVersion(
    { defaultBranch: 'main', headSha: 'a'.repeat(40) },
    null,
    async () => { branchRead = true; return 'unexpected'; },
  );
  assert.deepEqual(version, { baseSha: 'a'.repeat(40), targetBranch: 'main' });
