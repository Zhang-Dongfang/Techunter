...
当前入口覆盖 Desktop / 中央 API。旧版 CLI 直接操作 GitHub 的流程、GitHub 网页上的直接合并，以及已经发放的仓库凭据，不受这组 API 完整约束。需要真正文件级最小暴露时，应在后续改用由服务端按清单导出的任务快照或独立任务仓库，并避免把源仓库凭据和完整 Git 对象库交给接取者；sparse checkout 不能作为保密边界。
