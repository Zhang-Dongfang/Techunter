...
发布、认领、提交、释放和审核遇到中断时，可在任务详情恢复相应操作。无法继续的提交可撤回：核对并关闭未合并 PR 后重新交付，已合并且快照匹配的交付转回验收。聊天助手请求准备环境后由当前 Desktop 执行，并回报结果。验收与取消互斥；合并结果尚未确定时不会允许退款取消。
